...
import { Plus, Download, Edit, Trash2, X, Eye } from 'lucide-react'
    ...
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { setEditingClient(null); setFormData({ name: '', email: '', phone: '', address: '', city: '', state: '', zip_code: '', country: 'US' }); setSelectedCountry('US'); setShowModal(true) }} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"><Plus size={18} /> Add Client</button>
        </div>
      </div>
              ...
              {clients.map((client) => (
                <tr key={client.id} className="border-t">
                  ...
                  <td className="px-4 py-3 text-center space-x-2">
                    <Link href={`/dashboard/clients/${client.id}`} className="text-blue-600 hover:text-blue-800 inline-block">
                      <Eye size={18} />
                    </Link>
                    <button onClick={() => { setEditingClient(client); setFormData(client); setSelectedCountry(client.country || 'US'); setShowModal(true) }} className="text-green-600 hover:text-green-800 inline-block">
                      <Edit size={18} />
                    </button>
                    <button onClick={() => deleteClient(client.id)} className="text-red-600 hover:text-red-800 inline-block">
                      <Trash2 size={18} />
                    </button>
                  </td>
                </tr>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <select value={selectedCountry} onChange={(e) => setSelectedCountry(e.target.value)} className="p-2 border rounded-lg">
                  {COUNTRIES.map(c => <option key={c.code} value={c.code}>{c.name}</option>)}
                </select>
                
                {selectedCountry === 'US' && (
                  <select value={formData.state || ''} onChange={(e) => setFormData({...formData, state: e.target.value})} className="p-2 border rounded-lg">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input type="text" placeholder="City" value={formData.city || ''} onChange={(e) => setFormData({...formData, city: e.target.value})} className="p-2 border rounded-lg" />
                <input type="text" placeholder="Zip Code" value={formData.zip_code || ''} onChange={(e) => setFormData({...formData, zip_code: e.target.value})} className="p-2 border rounded-lg" />
              </div>
