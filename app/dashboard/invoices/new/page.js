...
function QuickInvoiceForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (description) invoiceData.notes = description
...
function CSVWizard() {
  ...
  const handleGenerateInvoices = async () => {
    ...
    try {
      ...
      toast.success(`${createdCount} invoices created!`)
      setTimeout(() => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      <div className="flex mb-6 border-b">
        <div className={`pb-2 px-3 ${step >= 1 ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-400'}`}>Step 1: Upload CSV</div>
        <div className={`pb-2 px-3 ${step >= 2 ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-400'}`}>Step 2: Map Columns</div>
        <div className={`pb-2 px-3 ${step >= 3 ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-400'}`}>Step 3: Review & Create</div>
      </div>
      
      {step === 1 && <CSVUploader onDataLoaded={handleDataLoaded} />}
      {step === 2 && csvData && <ColumnMapper columns={columns} sampleData={csvData.slice(0, 3)} onMappingComplete={handleMappingComplete} />}
      {step === 3 && mapping && csvData && (
        <div>
          <div className="mb-4 p-3 bg-gray-50 rounded-lg">
            <p>{csvData.length} invoices to create</p>
          </div>
          <button onClick={handleGenerateInvoices} disabled={processing} className="w-full bg-blue-600 text-white py-2 rounded-lg">
